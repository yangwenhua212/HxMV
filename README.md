...
| 质量检测 | **真测量 + 真看图** | L1 用 ffprobe 量分辨率/帧率/时长、blackdetect 量黑帧、freezedetect 量静止（**无音轨记为「没有音轨」而不是「音量过低」**——真 AI 视频默认无声，对不存在的音轨做增益是空操作）；**L2 抽 3 帧真画面 + 参考图**交给视觉模型判「是不是同一个角色/场景」（**只判给了参考的那一类**：只有角色图时不比场景，避免「换个雪景就被判不一致」的假失败）；**L3 抽 4 帧真画面**判「符不符合分镜」。没配 `HXMV_VLM_MODEL` 时 L2 退回 16×16 像素一致度、L3 退回文字判断，并在报告里明写「未做视觉检查」 |
