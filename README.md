...
> 经验进的是**起手参数**，不只是记事本：跑分里最常返工的身份/语义两类（`increase_reference_strength`、`rewrite_prompt_closer`）现在也带着验证过的经验起手（参考强度按验证次数 +0.1 起步、动作幅度、"提示词贴剧本"开关），物理旋钮（分辨率/帧率/音量/去黑场）同样起手就对。第四刀前后：首轮通过率 12%（平线）→ 38~50%，返工量与耗时继续下降（数据见 `docs/BENCH.md`）。
