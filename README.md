...
| 质量检测 | **真测量 + 真看图** | L1 用 ffprobe 量分辨率/帧率/时长、blackdetect 量黑帧、freezedetect 量静止（**音频默认无声**：没要音频时"有没有音轨"只作遥测、不判缺陷；任务明确要音频（`with_audio`）时缺音轨才记 `no_audio`——真 AI 视频默认无声，对不存在的音轨做增益是空操作，这是两种病）；**L2 抽 3 帧真画面 + 参考图**交给视觉模型判「是不是同一个角色/场景」（**只判给了参考的那一类**：只有角色图时不比场景，避免「换个雪景就被判不一致」的假失败）；**L3 抽 4 帧真画面**判「符不符合分镜」。没配 `HXMV_VLM_MODEL` 时 L2 退回 16×16 像素一致度、L3 退回文字判断，并在报告里明写「未做视觉检查」 |
...
> 经验进的是**起手参数**，不只是记事本：跑分里最常返工的身份/语义两类（`increase_reference_strength`、`rewrite_prompt_closer`）现在也带着验证过的经验起手（参考强度按验证次数 +0.1 起步、动作幅度、四条语义守卫：贴剧本/锁动作/锁情绪/锁衔接——每条守卫都真改发给模型的提示词，且进了画面指纹），物理旋钮（分辨率/帧率/音量/去黑场）同样起手就对。第四刀前后：首轮通过率 12%（平线）→ 38~50%，返工量与耗时继续下降（数据见 `docs/BENCH.md`）。
